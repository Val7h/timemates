...
print("\n" + "="*80)
print("POPULANDO BANCO NEON COM NOTÍCIAS E EVENTOS")
print("="*80 + "\n")

from database import SessionLocal, LocalNews, LocalEvent
...
try:
    ...
    db.query(LocalNews).delete()
    db.query(LocalEvent).delete()
    db.commit()
    print("[OK] Dados antigos removidos\n")

    print("[INFO] Inserindo 35 notícias...")
    for city, title, content, category in noticias:
        news = LocalNews(city=city, title=title, content=content, category=category, source="Local News")
        db.add(news)
    db.commit()
    print("[OK] Notícias inseridas\n")

    print("[INFO] Inserindo 35 eventos...")
    for city, title, description, date, time, location in eventos:
        event = LocalEvent(city=city, title=title, description=description, date=date, time=time, location=location, created_by_id=1, status="active")
        db.add(event)
    db.commit()
    print("[OK] Eventos inseridos\n")

    news_count = db.query(LocalNews).count()
    events_count = db.query(LocalEvent).count()

    print("="*80)
    print("✅ SUCESSO! BANCO POPULADO COM NOTÍCIAS E EVENTOS")
    print("="*80)
    print(f"\n📰 Notícias inseridas: {news_count}")
    print(f"🎪 Eventos inseridos: {events_count}")
    print(f"\n✨ Total: {news_count + events_count} registros\n")
