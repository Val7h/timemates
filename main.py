...
def _seed_user(db, full_name: str, email: str, city: str = None):
    """Cria usuário fictício ou retorna se já existe."""
    existing = db.query(User).filter(User.email == email).first()
    if existing:
        return existing
    u = User(
        full_name=full_name, email=email,
        password_hash=hash_password("Lourdes@2024"),
        cpf_hash=_hashlib.sha256(email.encode()).hexdigest(),
        city=city, is_active=True,
    )
    db.add(u); db.commit(); db.refresh(u)
    return u

def _seed_msg(db, room_id: int, user_id: int, content: str, days_ago: float):
    """Insere mensagem fictícia com timestamp no passado."""
    ts = datetime.utcnow() - timedelta(days=days_ago)
    existing = db.query(Message).filter(
        Message.room_id == room_id, Message.user_id == user_id,
        Message.content == content,
    ).first()
    if not existing:
        m = Message(room_id=room_id, user_id=user_id, content=content, created_at=ts)
        db.add(m)

def _seed_member(db, room_id: int, user_id: int):
    existing = db.query(RoomMembership).filter(
        RoomMembership.room_id == room_id, RoomMembership.user_id == user_id,
    ).first()
    if not existing:
        db.add(RoomMembership(
            ...
            role="member", status="approved", approved_at=datetime.utcnow(),
        ))
...
def seed_data(key: str = Form(...), db: Session = Depends(get_db)):
    ...
    if not school:
        school = Institution(
            ...
            approved=True, approved_by_id=admin_user.id,
        )
    ...
    rooms = {}
    for yr, label in decades:
        r = db.query(Room).filter(Room.institution_id == school.id, Room.year == yr).first()
        if not r:
            r = Room(
                institution_id=school.id, year=yr, group_name=label,
                description=f"Sala dos ex-alunos do Colégio Nossa Senhora de Lourdes — {label}. Venha reencontrar seus colegas!",
                created_by_id=admin_user.id,
            )
            db.add(r); db.commit(); db.refresh(r)
            _seed_member(db, r.id, admin_user.id)
            db.commit()
        rooms[yr] = r
    result["room_ids"] = {yr: r.id for yr, r in rooms.items()}

    # ── 3. Usuários fictícios ─────────────────────────────────────────────────
    seed = "@lourdes.palmares.seed"  # domínio fake, nunca vai receber email
    u80 = [
        _seed_user(db, "José Brito Santos",        f"jose.brito{seed}",        "Recife, PE"),
        _seed_user(db, "Maria Helena Cavalcante",  f"maria.helena{seed}",      "Palmares, PE"),
        _seed_user(db, "Antônio Ferreira Neto",    f"antonio.ferreira{seed}",  "São Paulo, SP"),
        _seed_user(db, "Conceição Lima Barros",    f"conceicao.lima{seed}",    "Caruaru, PE"),
        _seed_user(db, "Francisco Wanderley",      f"francisco.wanderley{seed}","Palmares, PE"),
    ]
    u90 = [
        _seed_user(db, "Carlos Eduardo Melo",      f"carlos.melo{seed}",       "Recife, PE"),
        _seed_user(db, "Patrícia Alves Gomes",     f"patricia.alves{seed}",    "João Pessoa, PB"),
        _seed_user(db, "Rodrigo Mendes Silva",     f"rodrigo.mendes{seed}",    "Palmares, PE"),
        _seed_user(db, "Ana Paula Barros",         f"ana.paula{seed}",         "Recife, PE"),
        _seed_user(db, "Marcelo Santos Lima",      f"marcelo.santos{seed}",    "Garanhuns, PE"),
    ]
    u00 = [
        _seed_user(db, "Thiago Oliveira Cruz",     f"thiago.oliveira{seed}",   "Recife, PE"),
        _seed_user(db, "Fernanda Costa Pereira",   f"fernanda.costa{seed}",    "São Paulo, SP"),
        _seed_user(db, "Lucas Brito Ramos",        f"lucas.brito{seed}",       "Palmares, PE"),
        _seed_user(db, "Juliana Almeida Santos",   f"juliana.almeida{seed}",   "Recife, PE"),
        _seed_user(db, "Rafael Nascimento",        f"rafael.nascimento{seed}",  "Caruaru, PE"),
    ]
    u10 = [
        _seed_user(db, "Pedro Henrique Rocha",     f"pedro.rocha{seed}",       "Recife, PE"),
        _seed_user(db, "Larissa Ferreira Lima",    f"larissa.ferreira{seed}",  "João Pessoa, PB"),
        _seed_user(db, "Gabriel Souza Melo",       f"gabriel.souza{seed}",     "Palmares, PE"),
        _seed_user(db, "Isabela Cruz Santos",      f"isabela.cruz{seed}",      "Recife, PE"),
        _seed_user(db, "Mateus Oliveira Gomes",    f"mateus.oliveira{seed}",   "Caruaru, PE"),
    ]
    u20 = [
        _seed_user(db, "João Victor Araújo",       f"joao.victor{seed}",       "Palmares, PE"),
        _seed_user(db, "Sofia Cardoso Ramos",      f"sofia.cardoso{seed}",     "Recife, PE"),
        _seed_user(db, "Miguel Lima Santos",       f"miguel.lima{seed}",       "Palmares, PE"),
        _seed_user(db, "Valentina Ferreira Cruz",  f"valentina.ferreira{seed}", "Olinda, PE"),
        _seed_user(db, "Davi Rodrigues Neto",      f"davi.rodrigues{seed}",    "Palmares, PE"),
    ]
    db.commit()

    # ── 4. Memberships ────────────────────────────────────────────────────────
    for u in u80: _seed_member(db, rooms[1980].id, u.id)
    for u in u90: _seed_member(db, rooms[1990].id, u.id)
    for u in u00: _seed_member(db, rooms[2000].id, u.id)
    for u in u10: _seed_member(db, rooms[2010].id, u.id)
    for u in u20: _seed_member(db, rooms[2020].id, u.id)
    db.commit()

    # ── 5. Mensagens fictícias ────────────────────────────────────────────────
    r80, r90, r00, r10, r20 = (
        rooms[1980].id, rooms[1990].id, rooms[2000].id, rooms[2010].id, rooms[2020].id
    )
    jose, maria_h, antonio, conceicao, francisco = (u.id for u in u80)
    carlos, patricia, rodrigo, ana_paula, marcelo = (u.id for u in u90)
    thiago, fernanda, lucas, juliana, rafael = (u.id for u in u00)
    pedro, larissa, gabriel, isabela, mateus = (u.id for u in u10)
    joao_v, sofia, miguel, valentina, davi = (u.id for u in u20)

    # Anos 80 — 3 a 4 dias atrás
    _seed_msg(db, r80, jose,      "Galera dos anos 80! Quem está aqui? Finalmente achei vocês! Saudades demais do Lourdes 🥹", 3.8)
    _seed_msg(db, r80, maria_h,   "José! Que surpresa boa! Tô em Palmares ainda, casada, com 3 filhos. O tempo passou rápido demais 😂", 3.6)
    _seed_msg(db, r80, antonio,   "Gente, procurei vocês no Orkut, no Facebook e não achei ninguém. Que bom que existe isso aqui! Fui pra São Paulo em 89 e a saudade só cresce", 3.4)
    _seed_msg(db, r80, conceicao, "Alguém lembra da Irmã Terezinha? Era brava mas aprendi muito com ela. E as aulas de moral e cívica... 😅", 3.2)
    _seed_msg(db, r80, francisco, "Lembro do uniforme azul com as listras até hoje. E o recreio no pátio, todo mundo correndo. Era outra época né", 3.0)
    _seed_msg(db, r80, jose,      "Vou estar em Palmares no São João! Quem tiver por lá, vamos nos encontrar na praça? 🎉", 2.8)
    _seed_msg(db, r80, maria_h,   "Estarei lá José! Vou levar meus filhos pra conhecer o colégio por fora. O prédio ainda está igual?", 2.6)
    _seed_msg(db, r80, antonio,   "Quero muito voltar um dia. Palmares fica no coração da gente, mesmo morando longe 🥹", 2.4)

    # Anos 90 — 2 a 3 dias atrás
    _seed_msg(db, r90, carlos,    "Oi pessoal! Carlos Eduardo, turma de 96! Alguém me reconhece? Cabelo tinha né, agora nem tanto 😂", 2.9)
    _seed_msg(db, r90, patricia,  "Carlos Eduardo! Meu Deus! Você era do time de futebol né? Como você está? Fui pra João Pessoa depois do Lourdes", 2.7)
    _seed_msg(db, r90, rodrigo,   "Turma dos 90 presente! Fiquei em Palmares, trabalho aqui ainda. A cidade mudou muito mas o colégio continua firme", 2.5)
    _seed_msg(db, r90, ana_paula, "Gente, a festa junina de 1997 foi a melhor de todas! Eu dancei quadrilha e quase caí no palco 😂🎉", 2.3)
    _seed_msg(db, r90, carlos,    "Ana Paula! Lembro sim! E o grêmio estudantil? A gente organizava tudo, era muito animado. Fui pra UFPE em 2001 depois", 2.1)
    _seed_msg(db, r90, marcelo,   "Pessoal, que memórias! Eu passei pro SESI em Recife e depois fiz Engenharia. Mas o Lourdes foi onde aprendi a estudar de verdade", 1.9)
    _seed_msg(db, r90, patricia,  "Que incrível Marcelo! Eu fiz Pedagogia. Quem mais foi pra universidade? Vamos contar a história de cada um 🥹", 1.7)
    _seed_msg(db, r90, rodrigo,   "Boa ideia Patrícia! E quem ficou em Palmares, como eu, também tem história pra contar 💪", 1.5)

    # Anos 2000 — 1 a 2 dias atrás
    _seed_msg(db, r00, thiago,    "Olá galera dos 2000! Thiago aqui, tentando reencontrar todo mundo. Quantos anos sem se ver! 🥹", 1.9)
    _seed_msg(db, r00, fernanda,  "Thiago! Lembro de você das aulas de química, sempre tirando nota boa 😂 Tô em São Paulo agora, trabalho em TI", 1.7)
    _seed_msg(db, r00, lucas,     "Eu fiquei em Palmares! O Lourdes mudou um pouco mas ainda tem aquela energia. Passo em frente todo dia", 1.5)
    _seed_msg(db, r00, juliana,   "Alguém tem contato do professor de história? O Zé Roberto? Ele ensinava muito bem, nunca esqueci as aulas dele", 1.3)
    _seed_msg(db, r00, thiago,    "Juliana! Acho que ele aposentou mas ainda mora aqui em Palmares. Posso tentar descobrir o contato", 1.1)
    _seed_msg(db, r00, rafael,    "Galera, alguém ainda tem foto da época? Eu tenho umas do Orkut salvas, de 2005-2006. Aquelas fotos de perfil constrangedoras 😂", 0.9)
    _seed_msg(db, r00, fernanda,  "Rafael manda as fotos!!!! Precisamos ver isso 😂😂😂", 0.7)
    _seed_msg(db, r00, lucas,     "Sim! Sobe as fotos aqui na galeria da sala! Essa plataforma tem espaço pra isso 📸", 0.5)

    # Anos 2010 — últimas 24h
    _seed_msg(db, r10, pedro,     "Formei em 2013! Quem mais está aqui? Fui pra UFPE logo depois e não voltei ainda 😅", 0.95)
    _seed_msg(db, r10, larissa,   "Pedro! Formei em 2014, fui pra UFPB. Palmares fica na saudade né. Como é a vida em Recife?", 0.8)
    _seed_msg(db, r10, gabriel,   "Eu fiquei em Palmares, trabalho aqui. Mas fico feliz de ver que o Lourdes fez diferença pra todo mundo 🙏", 0.65)
    _seed_msg(db, r10, isabela,   "Alguém lembra dos bazares que a gente fazia pro grêmio? Vendíamos bolo e coxinha na frente da escola 😂", 0.5)
    _seed_msg(db, r10, pedro,     "Isabela! Sim! E o dinheiro sumia misteriosamente antes de chegar no caixa 😂 Que saudades", 0.35)
    _seed_msg(db, r10, mateus,    "A gente cresceu demais né galera. Saudades daquela época sem responsabilidade... hoje tem conta pra pagar todo dia 😅", 0.2)
    _seed_msg(db, r10, larissa,   "Mateus falou tudo. Mas fico feliz que a gente se encontrou aqui! Precisamos marcar um reencontro em Palmares 🥹", 0.1)

    # Anos 2020 — hoje
    _seed_msg(db, r20, joao_v,    "Oi! João Victor, entrei no Lourdes em 2020. Quem mais pegou a pandemia logo no começo? Que começo de vida escolar difícil 😭", 0.4)
    _seed_msg(db, r20, sofia,     "Eu! As aulas online eram um caos total. Minha internet caía toda hora e a prof ficava braba 😂", 0.3)
    _seed_msg(db, r20, miguel,    "Mas valeu cada momento! Agora tô finalizando o ensino médio esse ano. Parece que passou rápido demais", 0.25)
    _seed_msg(db, r20, valentina, "Passei no ENEM e consegui vaga em Pedagogia em Olinda! Muito orgulho do Lourdes que me preparou 🎉🎉", 0.2)
    _seed_msg(db, r20, joao_v,    "Valentina que incrível!! Merecia demais! Eu ainda vou prestar vestibular esse ano, torçam por mim 🙏", 0.15)
    _seed_msg(db, r20, davi,      "Eu vou prestar também João Victor! Quero Medicina. Difícil mas vou tentar 💪", 0.1)
    _seed_msg(db, r20, valentina, "Davi vai conseguir sim! O Lourdes nos ensinou a não desistir 🥹 Vamos manter contato aqui", 0.05)

    db.commit()
    result["users_created"] = 25
    result["messages_created"] = "8 por sala (40 total)"
